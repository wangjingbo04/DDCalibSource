...
/*  // Define 1D histograms start values
  ...
  ih = analysisManager->CreateH1("h1.11", "Energy Spetrum from Filter 3", nbins, vmin, vmax);
  analysisManager->SetH1Activation(ih, true);
  ...
  //Histogram 13 - energy spectrum of neutrons entering liquid argon pool
  ih = analysisManager->CreateH1("h1.13", "Neutrons entering liquid Argon pool", nbins, vmin, vmax);
  analysisManager->SetH1Activation(ih, true);

  //Histogram 14 - Theta angle for neutrons exiting thermal absorber
  ih = analysisManager->CreateH1("h1.14", "Theta angle for neutrons entering the TPC", 360, xmin, xmax, "degree");
  analysisManager->SetH1Activation(ih, true);

  //Histogram 15 - Phi angle for neutrons exiting thermal absorber
  ...
  //Histogram 23 - Energy Spectrum of Neutrons Escaping Shield
  ih = analysisManager->CreateH1("h1.23", "Energy Spectrum of Neutrons Escaping Shield", nbins, minE, maxE, "MeV");
  analysisManager->SetH1Activation(ih, true);
